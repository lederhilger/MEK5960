...
In other words, it should be evanescent.
...
\begin{subequations}\label{eq:Helmholtz_decomposition}
        ...
        \end{equation}
\end{subequations}
We understand from the above discussion that this is just a special case of the \textsc{Hodge} decomposition, where $\Phi$ is a sharped zero-form, and $\mathbf{\Psi}$ is a sharped one-form.
...
where $\green$ is the familiar \textsc{Green} function, expressed in local coordinates as
\[
...
\noindent
Confer with \textsc{Xesin}.%
\footnote{%
        \cite{xesin2012symplectic} \textsc{Xesin} (2012)
}
