...
Alternatively to the above definition as generating functions, we have the definition through the hypergeometric function, as a solution to a special case of the hypergeometric equation,%
\footnote{%
...
\noindent
The hypergeometric function is a special function, whose full treatment is far beyond the scope of this work, but whose definition nonetheless warrants a mention for the sake of completion:%
\footnote{%
  \cite{abramowitz1965handbook} \textsc{Oberhettinger}, eq.15.1.1, p.556
}

\begin{equation}
  \hypergeometric(a, b; c; \ezh) = \frac{\Gamma(c)}{\Gamma(a)\Gamma(b)} \sum_{k = 0}^{\infty} \frac{\Gamma(a + k) \Gamma(b + k)}{\Gamma(c + k)} \frac{\ezh^k}{k!}
\end{equation}

\noindent
The ultraspherical functions are related to the associated \textsc{Legendre} spherical function%
\footnote{%
...
\noindent
Definition of associated \textsc{Legendre} functions%
\footnote{%
