...
We force regular solutions at $x = \pm 1$, yielding the eigenvalues $\lambda = m_1(m_1 + 1)$, such that $m_1 = \absl{m_0} + k$ for all nonnegative integers $k$.
...
It is customary to favor simple indices over the index chain for the hyperspherical harmonics, so that the upper index is $m$, and the lower index is $n$.
...
\begin{subequations}\label{eq:sphericalharmonics}
  \begin{equation}\tag{\theequation{a, b}}
    \spherical_{n}^{m}(\alpha^{0}, \alpha^{1}) = c_n^m \legendre_{n}^{\absl{m}}{(\cos{\alpha^1})} \exp{(im\alpha^0)}, \qquad c_n^m = {(-1)}^m \sqrt{\frac{2n+1}{4\pi} \frac{(n - \absl{m})!}{(n + \absl{m})!}}
  \end{equation}
\end{subequations}
